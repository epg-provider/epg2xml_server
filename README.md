# 알아두어야 할것
EPG 정보 가져오는 시간은 좀 바꿔주세요.
슬슬 이 프로그램이 알려지며넛 서버에 부하를 주는 것 같습니다.
대부분의 분들이 비슷한 시간대에 오게되면서 한번에 몰리는 것이 종종 문제를 일으키는 원인 중 하나입니다.
이미 문제가 생겨서 한번 프로그램과 소스를 변경한 적이 있으셔서 무슨 말인지 아실 겁니다.

데이터는 2틀치 정도만 가져오셔도 충분합니다.
최대 7일이 가능하지만 뒷부분 날짜일수록 정보가 정확하지 않을 확률이 높습니다.
게다가 이전에 등록된 내용은 변경되서 반영이 되지 않습니다.
7일씩 가져와봤자 서버에 부담만 주고 내용은 고작 가진 정보에 하루치가 업데이트 될 뿐입니다.
